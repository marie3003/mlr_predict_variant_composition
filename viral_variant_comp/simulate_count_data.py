...
def calculate_entropy(freq):
    ...
    return {"entropies": entropy_vec, "mean_entropy": mean_entropy}
...
def set_parameters(n_variants, delta_gr_range, rate, freq_entering_variants, s_0 = 0, o_0 = 0):
    ...
    return s_vec, o_vec, t

def sample(frequencies, n_samples):
    samples = np.array([np.random.multinomial(n_samples, row) for row in frequencies])
    return samples

def create_count_data(n_variants, delta_gr_range, new_var_rate, freq_entering_variants, n_samples, s_0, o_0, reorder = True, seed=None, entropy_cutoff = 1e-4):

    if seed is not None:
        np.random.seed(seed)
    
    s_vec, o_vec, t = set_parameters(n_variants = n_variants, delta_gr_range = delta_gr_range, rate = new_var_rate, freq_entering_variants = freq_entering_variants, s_0 = s_0, o_0 = o_0)

    frequencies = calculate_frequencies(t + 200, s_vec, o_vec)
    ...
    entropy_data = calculate_entropy(frequencies)
